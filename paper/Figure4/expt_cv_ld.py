...
sys.path.append('../..')
import os
os.makedirs('result_cv_ld', exist_ok=True)
    ...
    for b, batch in enumerate(batches):
        ...
        for i in batch:
            ...
            for j, n in enumerate(ns):
                ...
                res_klBSS_vanilla = KLBSS(Xt,Yt,s=s,betams=betamins,K=K,klbss_type='vanilla',cv=True)
                res_klBSS_simple = KLBSS(Xt,Yt,s=s,betams=betamins,K=K,klbss_type='simple',cv=True)

                f_klBSS_vanilla = open('result_cv_ld/Shat_klBSS_vanilla_' + string + '.txt', 'a')
                f_klBSS_vanilla.write(str(res_klBSS_vanilla) + '\n')
                f_klBSS_vanilla.close()

                f_klBSS_simple = open('result_cv_ld/Shat_klBSS_simple_' + string + '.txt', 'a')

...
def run_expt(d,s,s0,graph_type,err_dist,batch,b,IC):
    ...
    for i in batch:
        ...
        for j, n in enumerate(ns):
            ...
            res_BSS = KLBSS(Xt,Yt,ubs=ubs,ic=IC,bss=True,mip=True)
            res_klBSS = KLBSS(Xt,Yt,ubs=ubs,ic=IC,betam=betamin,mip=True)

            f_BSS = open(f'result_IC/{IC}/Shat_BSS_' + string + '.txt', 'a')
            f_BSS.write(str(res_BSS) + '\n')
            f_BSS.close()

            f_klBSS = open(f'result_IC/{IC}/Shat_klBSS_' + string + '.txt', 'a')
